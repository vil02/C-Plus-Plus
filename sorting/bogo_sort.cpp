 ...
 * algorithm](https://en.wikipedia.org/wiki/Bogosort)
 *
 * @details
 *      In computer science, bogosort (also known as permutation sort, stupid
 * sort, slowsort, shotgun sort, random sort, monkey sort, bobosort or shuffle
 * sort) is a highly inefficient sorting algorithm based on the generate and
 * test paradigm. Two versions of this algorithm exist: a deterministic version
 * that enumerates all permutations until it hits a sorted one, and a randomized
 * version that randomly permutes its input.Randomized version is implemented
 * here.
 *
 * ### Algorithm
...
/**
 * Implement randomized Bogosort algorithm and sort the elements of a given
 * array.
 * @tparam T typename of the array
 * @tparam N length of array
...
int main() {
    ...
    test();
    // Example Usage
    std::array<int, 5> arr = {3, 7, 10, 4,
                              1};  // Defining array which we want to sort
    std::cout << "Original Array : ";
    show_array(arr);
